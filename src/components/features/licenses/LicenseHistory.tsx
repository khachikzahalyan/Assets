    ...
    <div>
      ...
      {open && (
        <div className="mt-2 border border-border rounded-lg bg-bg p-3">
          {sorted.length === 0 ? (
            <p className="text-12 text-text-subtle">{t('historyEmpty')}</p>
          ) : (
              ...
              {sorted.map(entry => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="ml-2 text-11 text-text-subtle">
                    ...
                    {entry.after && typeof (entry.after as Record<string, unknown>).key === 'string' && (
                      <span className="ml-2 font-mono text-11 text-text-subtle">
                        {String((entry.after as Record<string, unknown>).key)}
                      </span>
                    )}
                  </div>
                  <span className="text-11 text-text-subtle flex-shrink-0 mt-0.5">
