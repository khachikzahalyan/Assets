...
export function SpecTile({ icon, label, value, accent, badge, badgeAccent, valueClassName, slots }: SpecTileProps) {
  ...
  return (
    <div className="bg-bg max-md:bg-surface border border-border rounded-lg p-3 flex items-center gap-3 max-md:p-2.5 max-md:gap-[0.5625rem]">
      ...
      <div className="min-w-0">
        <p className="text-10 max-md:text-9 tracking-widest max-md:tracking-[0.7px] max-md:font-bold text-text-subtle uppercase mb-0.5">{label}</p>
        {slots && slots.length > 0 ? (
            ...
            {slots.map((slot, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-1.5">
                  {slot.badge && (
                    <span
                      className={`inline-flex items-center rounded ring-1 px-1.5 py-0 text-10 font-semibold uppercase tracking-wider ${slotBadgeTone.bg} ${slotBadgeTone.text} ring-current/30`}
                    >
                      {slot.badge}
                    </span>
                  )}
                  <p className="text-13 max-md:text-12 max-md:font-semibold text-text-primary">{slot.value}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {badge && (
              <span
                className={`inline-flex items-center rounded ring-1 px-1.5 py-0 text-10 font-semibold uppercase tracking-wider ${badgeTone.bg} ${badgeTone.text} ring-current/30`}
              >
                {badge}
              </span>
            )}
            <p className={`text-13 max-md:text-12 max-md:font-semibold ${valueClassName ?? 'text-text-primary'}`}>{value}</p>
          </div>
        )}
      </div>
    </div>
