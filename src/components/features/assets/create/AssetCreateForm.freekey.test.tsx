...
async function fillIdentityAndWarehouse() {
  ...
  fireEvent.change(screen.getByPlaceholderText(/XPS 15, MacBook Pro/i), { target: { value: 'XPS 15' } })
  fireEvent.change(screen.getByPlaceholderText(/450\/100/i), { target: { value: '450/99' } })
  fireEvent.change(screen.getByPlaceholderText(/SN-XXXX/i), { target: { value: 'SN-PKR1' } })
...
describe('AssetCreateForm — free-OEM-pool picker (D7)', () => {
  ...
  it('picker-branch: selecting a free OEM license submits oemLicense: { existingLicenseId }', async () => {
    ...
    // Wait for identity fields + picker to appear
    await waitFor(() =>
  ...
  it('mutual-exclusivity: picking then typing raw key yields only rawKey in payload', async () => {
    ...
    fireEvent.change(rawKeyInput, { target: { value: 'ABCD-1234-EFGH-5678-IJKL' } })
    ...
    // The implementation's rule: typing in raw key clears picker → rawKey wins
    expect(oem).toEqual({ rawKey: 'ABCD-1234-EFGH-5678-IJKL' })
  })
  ...
  it('filter: picker lists only OEM + unassigned + active licenses', async () => {
    ...
    // Act — select OEM-capable category to trigger pool load
    const categorySelect = screen.getByRole('combobox', { name: /Категория/i })
    fireEvent.change(categorySelect, { target: { value: 'cat_laptop' } })
