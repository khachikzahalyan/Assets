    ...
    <div className="relative" ref={rootRef}>
      ...
      >
        ...
        <span className="ams-profile-name-text hidden sm:flex flex-col items-start leading-tight">
          <span className="text-12 font-semibold text-text-primary truncate max-w-[120px]">{user.name}</span>
          <span className="text-10 text-text-subtle">
              {t(`roles.${role}`, { ns: 'nav' })}
            </span>
        </span>
      ...
      {open && (
        <div
          ...
          className="ams-profile-dropdown absolute right-0 top-full mt-1.5 w-64 max-md:fixed max-md:top-[3.0625rem] max-md:right-0.5 max-md:left-auto max-md:bottom-auto max-md:mt-0 max-md:w-[220px] max-md:rounded-[14px] bg-surface-2 border border-border rounded-xl anim-dropdown-in overflow-hidden z-[200]"
        ...
        >
          ...
          <div className="flex flex-col items-center text-center px-3.5 pt-3 pb-2.5 border-b border-border">
            ...
            <div className="w-full text-13 font-semibold text-text-primary truncate leading-tight">{user.name}</div>
            <div className="w-full text-11 text-text-subtle truncate leading-tight">{user.email}</div>
          ...
          <div className="py-1">
            {canAccess(role, 'profile') && (
              <button
                ...
                className="w-full flex items-center gap-2.5 px-3.5 py-2 text-left text-12.5 text-text-secondary hover:bg-surface transition-colors"
              >
          ...
          <div className="border-t border-border py-1">
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3.5 py-2 text-left text-12.5 text-[#FDA4AF] light:text-rose-700 hover:bg-rose-950/40 light:hover:bg-rose-50 transition-colors"
            >
          ...
          {import.meta.env.DEV && (
            <div className="border-t border-border py-1">
              <div className="px-3.5 py-1.5 flex items-center gap-1.5">
                <span className="text-11 font-semibold text-text-subtle">{t('roleSwitcher.label')}</span>
