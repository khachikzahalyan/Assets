...
- [ ] **Step 1: Add keys** for: `title`, `auth.title`, `auth.subtitle`, `auth.domainsListLabel`, `auth.addLabel`, `auth.addPlaceholder`, `auth.addBtn`, `auth.saveBtn`, `auth.removeAria` (`{{domain}}`), `auth.empty`, `failClosed.banner`, `validation.invalid`, `validation.duplicate`, `validation.empty`, `saved`, `saveFailed`, `saving`, `confirm.title`, `confirm.body` (with `{{list}}`), `confirm.ok`, `confirm.cancel`, `confirm.saving`, `dangerConfirm.title`, `dangerConfirm.body`, `dangerConfirm.tokenLabel`, `dangerConfirm.token` (the literal token `DISABLE` — IDENTICAL across all locales), `dangerConfirm.confirmBtn`. Russian is the primary audience; provide real ru/en/hy translations (not English placeholders). (The final key set is driven by what `AuthSettingsPanel.tsx` actually references — a `loading.error` key was in an earlier draft but the panel uses `<ErrorState>` with its own copy, so it was dropped as dead.)
